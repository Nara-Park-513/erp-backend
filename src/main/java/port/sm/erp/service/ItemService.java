...
public class ItemService {
    ...
    */
    ...
    public ItemResponse create(ItemRequest req) {
        ...
        // 품목코드 중복체크
        if (itemRepo.existsByItemCode(req.getItem_code())) {
            throw new ResponseStatusException(CONFLICT, "이미 존재 하는 품목 코드입니다:  " + req.getItem_code());
        }
    ...
    private void validateRequired(ItemRequest req) {
        if (req.getItem_code() == null || req.getItem_code().isBlank())
            throw new ResponseStatusException(BAD_REQUEST, "품목코드는 필수입니다");
        if (req.getItem_name() == null || req.getItem_name().isBlank())
            throw new ResponseStatusException(BAD_REQUEST, "품목명은 필수입니다");
        if (req.getItem_type() == null)
    ...
    private void applyToEntity(Item item, ItemRequest req) {
        ...
        if (item.getId() == null) { // create
            item.setItemCode(req.getItem_code());
        }

        item.setItemName(req.getItem_name());
        item.setItemGroup(req.getItem_group());
        item.setSpec(req.getSpec());
        item.setSpecMode(req.getSpec_mode());
        item.setUnit(req.getUnit());
        item.setBarcode(req.getBarcode());
        item.setProcess(req.getProcess());

        item.setItemType(req.getItem_type());

        item.setIsSetYn(Yn.toYn(req.is_set()));
        item.setInPrice(req.getIn_price());
        item.setInVatIncludedYn(Yn.toYn(req.isIn_vat_included()));

        item.setOutPrice(req.getOut_price());
        item.setOutVatIncludedYn(Yn.toYn(req.isOut_vat_included()));

        item.setImageUrl(req.getImage_url());
        item.setUseYn(Yn.toYn(req.isUse_yn()));
    }
    ...
    private ItemResponse toResponse(Item item, Map<String, String> extraFields) {
        return ItemResponse.builder()
                ...
                .spec(item.getSpec())
                .spec_mode(item.getSpecMode())
                .unit(item.getUnit())
                .barcode(item.getBarcode())
                .process(item.getProcess())
                .item_type(item.getItemType())
                .is_set(Yn.toBool(item.getIsSetYn()))
                .in_price(item.getInPrice())
                .in_vat_included(Yn.toBool(item.getInVatIncludedYn()))
                .out_price(item.getOutPrice())
                .out_vat_included(Yn.toBool(item.getOutVatIncludedYn()))
                .image_url(item.getImageUrl())
                .use_yn(Yn.toBool(item.getUseYn()))
